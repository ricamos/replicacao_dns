...
Vagrant.configure("2") do |config|
  ...
  config.vm.define :ns1 do |ns1_config|
    ...
    ns1_config.vm.provision "shell", path: "files/ns1.sh"                      
  end
  ...
  config.vm.define :ns2 do |ns2_config|
    ...
    ns2_config.vm.provision "shell", path: "files/ns2.sh"                      
  end

  config.vm.define :fw1 do |fw1_config|
    fw1_config.vm.hostname = "fw1"
    fw1_config.vm.network :private_network,
                          :ip => "192.168.42.10"
    fw1_config.vm.network :private_network,
                          :ip => "172.16.42.10"
    fw1_config.vm.provision "shell", path: "files/fw1.sh"                                              
  end                        
end
